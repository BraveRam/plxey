...
- **Design system** (`src/index.css`): minimal **flat hairline** style — every surface is a flat `--card` fill + a single 1px `--border`, no rings, no shadows (the lone exception is `--ds-island-shadow`, reserved for the floating bottom CTA / filled primary buttons). No grain/orbs; `--radius` ≈ 14px on a tight scale; plain uppercase eyebrows; subtle opacity+rise `Reveal`. The `Bezel` primitive renders one flat hairline card. Recessed tiles inside a card use a `bg-foreground/[0.0x]` tint (not `--muted`) so they stay distinct even when Telegram maps `--muted == --card`.
- **Telegram bridge** (`src/lib/telegram.ts`): `WebApp.ready()/expand()`, maps `themeParams` onto shadcn CSS variables (light/dark follows `colorScheme`), exposes signed `initData`, native BackButton, haptics. Because the flat design is border-load-bearing and Telegram may send `secondary_bg_color == bg_color` (collapsing `--card` onto `--background`), `applyTheme` **derives `--border`/`--input` from the resolved palette** (`color-mix(in oklch, var(--foreground) 12%, var(--card))`) so the hairline is always visible in any client theme.
...
- **Deep-link routing**: `App.tsx:StartParamRouter` reads `WebApp.initDataUnsafe.start_param` on mount and routes via `START_PARAM_ROUTES` (`billing` → `/billing`). Notification DMs from `notify-owner.ts` embed `t.me/<onboarding-bot>?startapp=billing` URL buttons so owners can jump straight to the richer Mini App billing surface from any lifecycle DM. (Note: `?startapp=` deep links require a BotFather-registered Main Mini App this bot lacks, so these billing links currently fail with "this application doesn't exist" until that's configured or they're switched to inline `web_app` buttons. The admin dashboard sidesteps this entirely — it's an in-app button, not a deep link.)
...
**process** — `splitText(text)` with `size=500`, `overlap=50`. Recursive descent through separators `["\n\n", "\n", ".", "?", "!", ",", " ", ""]`. Splits at each level; if a piece exceeds 500 chars, recurses to next separator. `""` is the final character-level fallback. Overlap pass prepends last 50 chars of previous chunk to next.

A doc that splits into more than `MAX_CHUNKS` (5000) throws a `NonRetriableError` → caught below → `status='failed'` with no retries (a pathological upload fails fast instead of embedding for minutes). Otherwise the chunks are embedded **and inserted in batches** of `INGEST_BATCH_SIZE` (50) via `batchRows` (`apps/rag/src/batch.ts`): per batch — re-read `status` (cancel guard; `cancelOn` only aborts between Inngest steps and this loop is one step), `embedMany` the 50-chunk slice, then `insert(...).values(rows).onConflictDoNothing()` with `chunkIndex = batchStart + offset` and `tenantBotId = botId`. Batching is load-bearing: a single all-chunks insert builds one multi-MB statement (each row carries a 1536-dim vector ≈ 24KB as text) that exceeds the **Neon HTTP request-size limit** — the prod symptom was a `DrizzleQueryError: Failed query` on `document_chunks` for a multi-MB upload. `onConflictDoNothing` (against `document_chunks_doc_chunk_uq`) makes batches idempotent so a step retry after a partial insert is a no-op, not a unique violation.

**finish** — `documents.status = 'ready'`, fetch `tenants.telegramOwnerId` + `tenantBots.botTokenEncrypted`, decrypt, raw `fetch` to Telegram `sendMessage` to notify the owner. On any exception in any step: catch → log the underlying driver `cause` (the real Neon/Postgres reason; `DrizzleQueryError.message` only carries the SQL + params, so the cause was previously lost on rethrow and ingest failures were undiagnosable) → set `documents.status = 'failed'` → re-throw (Inngest retries non-`NonRetriableError` failures).
...
- **Not admin-gated** (any authenticated caller). Returns `{ ownerId, isAdmin }` where `isAdmin = isAdminOwnerId(ownerId, ADMIN_TELEGRAM_USER_ID)`. The Mini App home screen (`BotList`) calls this to decide whether to render the admin entry button next to billing. Reveals only the caller's own status, never the admin id; the real protection stays on `/api/admin/*`.
...
| `ADMIN_TELEGRAM_USER_ID` | Recommended (prod) | Telegram user id allowed to run admin commands (`/owner`, `/refund`, `/grant_comp`, `/revoke_comp`, `/ban`, `/unban`, `/broadcast`) **and** the admin dashboard (`/api/admin/*` routes + the `GET /api/me`-gated admin button in the Mini App). Fail-closed if unset. | Generated |
...
- `pre_checkout_query` validation (payload format, banned, already-subscribed-same-plan, nonce dedup). New subscriptions only — Telegram does not send `pre_checkout_query` for recurring renewals, so renewal amount validation happens entirely in the `successful_payment` handler.
- `message:successful_payment` handler that records the ledger row and fires `subscription/started` or `subscription/renewed`.
  - **Amount validation is grandfather-aware** (`resolveExpectedStars`). First / non-recurring payments must equal the **catalog** price (`PLANS[plan].starsPerPeriod`) — the price baked into the invoice we minted. Renewals (`is_recurring && !is_first_recurring`) are auto-charged by Telegram at the subscription's **locked-in** price, which can differ from the catalog after a price change — Telegram has no API to reprice a live Stars subscription, so existing subscribers are grandfathered. Renewals validate against the subscription row's stored `starsPerPeriod`, located by the recurring chain's `telegram_payment_charge_id`. If the row can't be found, validation falls back to the catalog price (defense-in-depth against forged renewals). **Without this, changing any plan's price would cause every existing subscriber's next renewal to fail the amount check ("refusing to grant access") even though Telegram charged them correctly.**
...
- `ADMIN_TELEGRAM_USER_ID` — Telegram user id for the admin command surface **and** the admin dashboard (`/api/admin/*` routes + the `GET /api/me`-gated admin button → Mini App `/admin` route). If unset, the admin surface is silently denied (fail-closed).
...
Single-operator analytics surface. The admin opens the Mini App normally (the onboarding bot's chat menu button), and the home screen (`BotList`) shows an extra **admin button next to billing** in the header — rendered only when `GET /api/me` reports `isAdmin` (so non-admins never see it). Tapping it navigates in-app to `/admin`. There is no `/dashboard` slash command and no `?startapp=` deep link (that would need a BotFather-registered Main Mini App the bot doesn't have — "this application doesn't exist"). Everything is gated to `ADMIN_TELEGRAM_USER_ID`; the button is cosmetic — the server re-verifies the admin id on every `/api/admin/*` request, and a non-admin who reaches `/admin` directly gets a 403 → `Forbidden` wall.
